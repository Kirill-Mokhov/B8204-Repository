...
short opz::get_priority(const char sign) {
	switch (sign) {
	case '^':
		return 3;
	case '*':
...
const char *opz::get_expression(sign_stack_t &stack) {
...
std::string opz::convert(std::string input) {
...
double opz::calculate(std::string opz) {
	double num, n1, n2, res;
	char *temp = new char[opz.size()];
	strcpy(temp, opz.c_str());
