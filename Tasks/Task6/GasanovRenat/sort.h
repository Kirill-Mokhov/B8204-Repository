...
class ShakerSort: public SortTool{
public:
    ShakerSort(CompareInterface& _compareTool, SwapInterface& _swapTool): SortTool(_compareTool, _swapTool){};
    void Sort(int *Arr, unsigned int Size) {
        ...
        while (Left <= Right){
            for (int i = Right; i >= Left; i--)
                if (compareTool->Compare(Arr[i], Arr[i-1]) == 1) swapTool->Swap(Arr[i], Arr[i-1]);
            Left++;
